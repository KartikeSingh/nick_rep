...
module.exports = {
                        ...
                        options: [
                            {
                                ...
                                type: 6,
                                description:
                                    "User who's reps you want to renew",
                                required: true,
                            },
                        ],
                    },
                        ...
                        options: [
                            {
                                ...
                                type: 6,
                                description:
                                    "User who's reps you want to increase",
                                required: true,
                            },
                            {
                                name: "amount",
                            ...
                            },
                            {
                                name: "silent",
                                type: 5,
                                description: "😳 Make it silent",
                            },
                            {
                                name: "staff",
                                type: 5,
                                description: "Is this a staff rep?",
                            },
                        ],
                    },
                ],
    ...
    run: async (client, interaction) => {
        ...
        const reason =
            interaction.options.getString("reason") || "Admin Add Command";
        const mode = interaction.options.getInteger("mode");
        const id = interaction.options.getString("id");
        const silent = interaction.options.getBoolean("silent");
        const staff = interaction.options.getBoolean("staff");
        ...
        if (group === "rep") {
            if (option === "delete") {
                    ...
                    return interaction.editReply({
                        embeds: [
                            new EmbedBuilder()
                                ...
                                .setDescription(
                                    `The reputation with the ID \`${id}\` was not found.`
                                ),
                        ],
                    });
                ...
                interaction.editReply({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setDescription(
                                `The reputation with the ID \`${id}\` was deleted.`
                            ),
                    ],
                });
            } else if (option === "wipe") {
                if (mode) {
                    ...
                    await reputationModel.updateMany(
                        { target: user.id },
                        {
                            isValid: false,
                            invalidatedBy: interaction.user.id,
                            "reason.remove": "Wipe Command",
                        }
                    );
                }
                ...
                await userModel.findOneAndUpdate(
                    { id: user.id },
                    { reputation: 0, staffReputation: 0 },
                    { upsert: true }
                );
                ...
                interaction.editReply({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setDescription(
                                `The reputation of \`${user.tag}\` was ${
                                    mode ? "deleted" : "wiped"
                                }.`
                            ),
                    ],
                ...
                await reputationModel.updateMany(
                    { target: user.id, isValid: false },
                    {
                        isValid: true,
                        revalidatedBy: interaction.user.id,
                        "reason.remove": "Renew Command",
                    }
                );
                ...
                await userModel.findOneAndUpdate(
                    { id: user.id },
                    {
                        reputation: await reputationModel.countDocuments({
                            target: user.id,
                            isValid: true,
                        }),
                        staffReputation: await reputationModel.countDocuments({
                            target: user.id,
                            isValid: true,
                            staff: true,
                        }),
                ...
                interaction.editReply({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setDescription(
                                `The reputation of \`${user.tag}\` was renewed.`
                            ),
                    ],
                });
            } else if (option === "add") {
                    ...
                    return interaction.editReply({
                        embeds: [
                            new EmbedBuilder()
                                ...
                                .setDescription(
                                    "Please add some reputation categories first!"
                                ),
                        ],
                    });
                ...
                const minecraftId = (
                    await userMincraftModel.findOne(
                        { id: user.id },
                        { uuid: 1 }
                    )
                )?.uuid;
                ...
                for (let i = 0; i < amount; i++) {
                    ...
                    const admin = silent
                        ? null
                        : await client.log.admin.send({
                              embeds: [
                                  new EmbedBuilder()
                                      .setTitle("⭐ New Reputation Added")
                                      .setDescription(
                                          `💼 **Admin Command (${interaction.user.username}/${interaction.user.id})**`
                                      )
                                      .addFields(
                                          {
                                              name: "Sender",

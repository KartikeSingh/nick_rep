...
module.exports = {
    ...
    run: async (client, interaction) => {
        const newMemberTill =
            interaction.member.joinedTimestamp + 2 * 24 * 3600 * 1000;

        if (newMemberTill > Date.now())
            return interaction.reply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setTitle("🍼 Too Young")
                        .setDescription(
                            `You can give reputations <t:${Math.floor(
                                newMemberTill / 1000
                            )}:R>`
                        ),
                ],
        ...
        const senderData =
            (await userModel.findOne({ id: interaction.user.id })) ||
            (await userModel.create({ id: interaction.user.id }));

        if (senderData.banned)
            return interaction.editReply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription(
                            "You are banned from giving reputation"
                        ),
                ],
            });
        ...
        if (
            globalTimeoutTill > Date.now() &&
            !interaction.member.roles.cache.has(process.env.COMMUNITY_ROLE)
        )
            return interaction.editReply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setTitle("⏰ Timeout")
                        .setDescription(
                            `You can give next reputation again <t:${Math.floor(
                                globalTimeoutTill / 1000
                            )}:R>`
                        ),
                ],
            });
            ...
            return interaction.editReply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setTitle("⏰ Timeout")
                        .setDescription(
                            `You can give reputation to ${user.toString()}, again <t:${Math.floor(
                                userTimeoutTill / 1000
                            )}:R>`
                        ),
                ],
            });
        ...
        const guildData =
            (await guildModel.findOne({ id: interaction.guildId })) ||
            (await guildModel.create({ id: interaction.guildId }));
            ...
            return interaction.editReply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription(
                            "Please contact admins to add reputation categories"
                        ),
                ],
            });

        const [newInteraction, category] = await chooseCategory({
            interaction,
            guildData,
        });
            ...
            return interaction.editReply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setTitle("❌ Error")
                        .setDescription(
                            "You took too long in selecting reputation category"
                        ),
                ],
            });

        const [newInt2, reason] = await chooseReason({
        ...
        const staff = interaction.member.roles.cache.has(
        ...
        await userModel.updateOne(
            ...
            {
                [`timeouts.${receiverData.id}`]: Date.now() + 48 * 3600 * 1000,
                [`timeouts.lastRep`]: Date.now() + 600000,
            }
        );
        await userModel.updateOne(
            { id: user.id },
            { $inc: { reputation: 1, staffReputation: staff ? 1 : 0 } }
        );

        const repId = randomID(14, ["letter", "number"]);
        const minecraftId = (
            await userMincraftModel.findOne({ id: user.id }, { uuid: 1 })
        )?.uuid;
        const likelyAbuse =
            (await reputationModel.countDocuments({
                target: user.id,
                createdAt: { $gte: Date.now() - 1 * 3600 * 1000 },
                isValid: true,
            })) > 20;
        ...
        const admin = await client.log.admin.send({
            embeds: [
                new EmbedBuilder()
                    ...
                    .setThumbnail(
                        `https://crafatar.com/renders/body/${minecraftId}`
                    )
                    .setTimestamp(),
            ],
            components: [
        ...
        const public = await client.log.public.send({
            embeds: [
                new EmbedBuilder()
                    ...
                    .setDescription(
                        `${interaction.user.toString()} has given ${user.toString()} a rep!\n${user.toString()} now has \`${
                            receiverData.reputation + 1
                        }\` reps!`
                    )
                    .setColor("Green")
                    ...
                    // })
                    .setThumbnail(
                        `https://crafatar.com/renders/body/${receiverData.minecraftId}`
                    )
                    .setTimestamp(),
            ],
        });

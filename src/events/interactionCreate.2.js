...
module.exports = async (client, interaction) => {
    ...
    if (type === "lookup") {
        if (!member.permissions.has("ManageGuild") && !client.owners.includes(interaction.user.id) && !member.roles.cache.has("1081482836470673428")) return interaction.reply({
            embeds: [
                new EmbedBuilder({
                    title: "❌ Not Allowed",
        ...
        if (!u) return interaction.editReply({
            embeds: [
                new EmbedBuilder()
                    .setColor("Red")
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({
                ...
                name: "Last Updated",
                value: userData.lastUpdated ? `<t:${Math.floor(userData.lastUpdated / 1000)}:R>` : "`Never`",
                inline: true
            });
        ...
        interaction.editReply({
            embeds: [
                userRepEmbed({
                    ...
                    reps: await reputationModel.find({ target: id, isValid: true }, { category: 1 }).lean(),
                    viewer: interaction.user
                })
            ]
        });
    } else if (type === "validate") {
        if (!member.roles.cache.has(process.env.STAFF_ROLE)) return interaction.reply({
            embeds: [
                new EmbedBuilder({
                    title: "❌ Not Allowed",
        ...
        if (data.isValid) return interaction.editReply({
            embeds: [
                new EmbedBuilder()
                    .setColor("Red")
        ...
        const userData = await userModel.findOneAndUpdate({ id: data.target }, { $inc: { reputation: 1, staffReputation: data.staff ? 1 : 0 } }, { new: true });
        const repData = await reputationModel.findOneAndUpdate({ id }, { isValid: true, revalidatedBy: interaction.user.id, updatedAt: Date.now() }, { new: true });
        ...
        if (!data.isValid) return interaction.followUp({
            ephemeral: true,
            embeds: [
                new EmbedBuilder()
                    .setColor("Red")
        ...
        const userData = await userModel.findOneAndUpdate({ id: data.target }, { $inc: { reputation: -1, staffReputation: data.staff ? -1 : 0 } }, { new: true });
        const repData = await reputationModel.findOneAndUpdate({ id }, { isValid: false, invalidatedBy: interaction.user.id, updatedAt: Date.now(), "reason.remove": reason }, { new: true });
